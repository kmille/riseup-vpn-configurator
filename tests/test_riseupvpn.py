...
class TestRiseupVPN:
    ...
    def test_get_rtt_valid(self):
        from riseup_vpn_configurator import get_rtt
        rtt = get_rtt("1.1.1.1")
        assert type(rtt) is float

    def test_get_rtt_invalid(self):
        from riseup_vpn_configurator import get_rtt
        rtt = get_rtt("1.1.1.1brokenip")
        assert type(rtt) is float
        assert rtt == 9000.0
    ...
    def test_list_gateways_with_benchmark(self, capsys):
        ...
        list_gateways(True)
        captured = capsys.readouterr()
        assert "vpn01-sea.riseup.net location=Seattle       ip=204.13.164.252  rtt=" in captured.out
